...
impl<'d> Client<'d>{
    ...
    pub async fn connect_tls<R: RngCore + CryptoRng>(
        ...
        socket.connect(endpoint).await.map_err(|_| ClientError::NotConnected)?;
        let tls_config = self.build_tls_config(server_ca, identity)?;
        let tls_context = TlsContext::new(&tls_config,rng);
        let mut tls_connection = TlsConnection::<'d>::new(
            socket,
            tls_rx_buffer,
            tls_tx_buffer
        );
        tls_connection.open::<_, NoVerify>(tls_context).await.map_err(ClientError::from)?; // need a TLSverifier 
        ...
        &mut self,
         data: &[u8]
            ...
            match &mut self.tls_connection {
                Some(tls) => {
                    tls.write(data).await.map_err(ClientError::from)
                }
